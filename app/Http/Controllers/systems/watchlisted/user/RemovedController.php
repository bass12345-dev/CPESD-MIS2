...
class RemovedController extends Controller
{
    ...
    public function index(){
        $data['title']                      = 'Removed';
        return view('systems.watchlisted.user.pages.removed.removed')->with($data);
    }

    public function get_removed_watchlisted(){

        $items       = $this->customRepository->q_get_where_order($this->conn,'persons',array('added_by' => session('user_id'),'status'=>'inactive'),'created_at','desc')->get();
        $i = 1;
        $data = [];
